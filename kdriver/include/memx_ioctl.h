...
/*
 * IMPORTANT:
 * Keep this header identical across kernel/user include paths.
 * If you must add user-friendly typedefs, ONLY alias canonical tags.
 * Do NOT re-declare struct bodies with different names.
 */
...
enum ADMIN_CMD_OPCODE {
    ...
    MEMX_ADMIN_CMD_DEVIOCTRL     = 0x5,
    MEMX_ADMIN_CMD_FLASH_RW      = 0x6,
    MEMX_ADMIN_CMD_MAX
};
...
enum memx_pcie_bar_mode {
    ...
    MEMXBAR_4BAR_BAR0VB_BAR2CI_BAR4MSIX_BAR5SRAM = 5,
...
struct fw_info {
    unsigned int bar0_mapping_mpu_base;          /* fixed 0x30000000 */
    unsigned int bar1_mapping_sram_base;         /* fixed 0x40000000 */

    unsigned int firmware_download_sram_base;    /* fixed 0x40040000 */
    unsigned int firmware_command_sram_base;     /* fixed 0x40046E00 */
    unsigned long long rx_dma_coherent_buffer_base;
...
struct memx_mpu_size {
    ...
    unsigned int flow_size[MEMX_TOTAL_FLOW_COUNT];
...
enum CMD_FLASH_ACCESS_ID {
  FLASH_ACCESS_ID_READ  = 0x0,
  FLASH_ACCESS_ID_WRITE = 0x1,
  FLASH_ACCESS_ID_ERASE = 0x2,
  FLASH_ACCESS_ID_MAX
};
...
struct transport_cmd {
	union{
		...
		struct transport_sq_flash SQ_flash;
	};
	union{
...
#define MEMX_EXTINFO_CMD(chip_id, cmd_code, access_mode) \
    ((unsigned int)(((chip_id) << 28) | ((access_mode) << 4) | (cmd_code)))
...
// device files
#define MEMX_PCIE_SINGLE_DEV_FILE      "/dev/memx"      /* PCIE single device */
#define MEMX_PCIE_SPLIT_IN_DEV_FILE    "/dev/memxi"     /* PCIE cascade plus in device */
#define MEMX_PCIE_SPLIT_OUT_DEV_FILE   "/dev/memxo"     /* PCIE cascade plus out device */

#define MEMX_USB_SINGLE_DEV_FILE       "/dev/memxchip"  /* USB single device */
#define MEMX_USB_SPLIT_IN_DEV_FILE     "/dev/memxchipo" /* USB cascade plus bulkin device */
#define MEMX_USB_SPLIT_OUT_DEV_FILE    "/dev/memxchipi" /* USB cascade plus bulkout device */
...
#define MEMX_DEVICE_IRQ					CTL_CODE(FILE_DEVICE_UNKNOWN,	   \
													MEMX_IOCTL_INDEX + 22, \
													METHOD_BUFFERED,	   \
													FILE_ANY_ACCESS)

#define MEMX_WRITE_REG                  CTL_CODE(FILE_DEVICE_UNKNOWN,      \
                                                    MEMX_IOCTL_INDEX + 23, \
                                                    METHOD_BUFFERED,       \
                                                    FILE_ANY_ACCESS)
...
typedef struct memx_device_irq_param {
    unsigned char chip_id;
    unsigned int irq_id;
} memx_device_irq_param_t;
