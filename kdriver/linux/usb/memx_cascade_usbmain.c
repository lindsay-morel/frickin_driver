...
static u32 g_drv_fs_type = MEMX_FS_HIF_DEVICE;
...
#endif
{
	...
	if (MAJOR(dev->devt) != MAJOR(g_feature_devno))
		return NULL;

	return kasprintf(GFP_KERNEL, "memxchip%d_feature", MINOR(dev->devt));
}
...
static int memx_get_hw_info(struct memx_data *data, void *kbuf, unsigned long n_bytes)
{
	uint32_t cfg_header[2] = {0};
	uint32_t xfer_total_size = n_bytes;
	uint32_t buffer_addr = MEMX_PCIE_CMD_BUF_STARTADDR;

	if (n_bytes > MAX_OPS_SIZE)
		return -EINVAL;
	...
	memcpy(kbuf, data->fw_rbuffer, xfer_total_size);
	return 0;
}
...
static void memx_usb_update_mpu_groups_info(struct hw_info *hw_info)
{
	...
	memset(hw_info->chip.groups, 0, sizeof(hw_info->chip.groups));
...
static void memx_usb_init_hw_info(struct memx_data *data)
{
	struct fw_hw_info_pkt hw_info;
	int ret;
	uint8_t chip_id;

	memset(&data->hw_info, 0, sizeof(data->hw_info));
	memset(&hw_info, 0, sizeof(hw_info));

	ret = memx_get_hw_info(data, &hw_info, sizeof(hw_info));
	if (ret) {
		pr_err("%s: Get hardware info error: %d", __func__, ret);
		return;
	}

	data->hw_info.chip.total_chip_cnt = (uint8_t)(hw_info.total_chip_cnt & 0xff);
	for (chip_id = 0; chip_id < MAX_SUPPORT_CHIP_NUM; chip_id++)
		data->hw_info.chip.roles[chip_id] = hw_info.chip_role[chip_id];
	memx_usb_update_mpu_groups_info(&data->hw_info);
}
...
static long memx_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MEMX_GET_HW_INFO:
		if (copy_from_user(&hw_info, (struct hw_info *)arg, sizeof(struct hw_info))) {
			pr_err("MEMX_GET_HW_INFO copy_from_user fail\n");
			ret = -ENOMEM;
			break;
		}
		data->hw_info.chip.generation = hw_info.chip.generation; // keep chip generation from user config
		if (copy_to_user((void __user *)arg, &data->hw_info, sizeof(struct hw_info))) {
			pr_err("MEMX_GET_HW_INFO copy_to_user fail\n");
			ret = -ENOMEM;
			break;
		}
	break;
	case MEMX_CONFIG_MPU_GROUP:
		...
		memcpy(&data->hw_info.chip, &hw_info.chip, sizeof(data->hw_info.chip));
...
static int memx_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	struct usb_class_driver *chosen = NULL;
	s32 ret = 0;
	bool usb_dev_registered = false;
	...
	switch (id->idProduct) {
	case ROLE_G0_MULTI_FIRST:  chosen = &memxchip_g0classo; break;
	case ROLE_G1_MULTI_FIRST:  chosen = &memxchip_g1classo; break;
	case ROLE_G2_MULTI_FIRST:  chosen = &memxchip_g2classo; break;
	case ROLE_G3_MULTI_FIRST:  chosen = &memxchip_g3classo; break;
	case ROLE_G0_MULTI_LAST:   chosen = &memxchip_g0classi; break;
	case ROLE_G1_MULTI_LAST:   chosen = &memxchip_g1classi; break;
	case ROLE_G2_MULTI_LAST:   chosen = &memxchip_g2classi; break;
	case ROLE_G3_MULTI_LAST:   chosen = &memxchip_g3classi; break;
	case ROLE_G1_SINGLE_DEV:   chosen = &memxchip_g1class;  break;
	case ROLE_G2_SINGLE_DEV:   chosen = &memxchip_g2class;  break;
	case ROLE_G3_SINGLE_DEV:   chosen = &memxchip_g3class;  break;
	case ROLE_MX3PLUS_ZSBL_DEV:
		/* keep your existing special flow, but make sure cleanup is correct if it fails */
		ret = memx_firmware_init(data);
		if (ret) goto err_free_alloc;
		return 0;
	default:
		chosen = &memxchip_g0class; break;
	}
	ret = usb_register_dev(intf, chosen);
	if (ret) {
		pr_err("usb_register_dev failed: %d\n", ret);
		goto err_free_alloc;
	}
	data->class_driver = chosen;
	usb_dev_registered = true;
	memx_usb_init_hw_info(data);

	mutex_lock(&device_mutex);
	if (check_device_all_empty() == true) {
		ret = alloc_chrdev_region(&g_feature_devno, 0, MAX_CHIP_NUM, DEVICE_CLASS_NAME);
		if (ret < 0) {
		...
		memx_feature_class = class_create(THIS_MODULE, DEVICE_CLASS_NAME);
#else
		memx_feature_class = class_create(DEVICE_CLASS_NAME);
#endif
		if (IS_ERR(memx_feature_class)) {
			ret = PTR_ERR(memx_feature_class);
	...
	ret = cdev_add(&data->feature_cdev, MKDEV(MAJOR(g_feature_devno), intf->minor), 1);
	if (ret) {
		pr_err("Can't add feature cdev %d\n", ret);
		mutex_unlock(&device_mutex);
		goto err_free_alloc;
	}
	pDevice = device_create(memx_feature_class, &data->interface->dev, MKDEV(MAJOR(g_feature_devno), intf->minor), data, DEVICE_NODE_NAME, intf->minor);
	if (IS_ERR(pDevice)) {
		ret = PTR_ERR(pDevice);
...
err_free_alloc:
	if (usb_dev_registered)
		usb_deregister_dev(intf, chosen);
	if (data->interface)
	...
	if (data->tbuffer) kfree(data->tbuffer);
	if (data->product_id != ROLE_MX3PLUS_ZSBL_DEV) {
		...
		if (data->fw_rbuffer) kfree(data->fw_rbuffer);
		if (data->fw_wbuffer) kfree(data->fw_wbuffer);
...
static void memx_disconnect(struct usb_interface *intf)
{
	...
	if (data->class_driver)
		usb_deregister_dev(intf, data->class_driver);
