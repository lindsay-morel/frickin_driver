...
#define STATUS_COPY_ERROR (4)

static int memx_feature_read_chip0(struct memx_data *data, uint32_t *buffer, uint32_t address, uint32_t size);
static uint32_t memx_admin_fetch_result(struct transport_cmd *pCmd, struct memx_data *data);
static uint32_t _admin_flash_access(struct transport_cmd *pCmd, struct memx_data *data);
...
int memx_admin_trigger(struct transport_cmd *pCmd, struct memx_data *data)
{
        ...
        if (!wait_for_completion_timeout(&data->fw_comp, msecs_to_jiffies(MXCNST_TIMEOUT30S))) {
            ...
            if (usb_submit_urb(data->fw_txurb, GFP_KERNEL) < 0) {
                ...
                if (!wait_for_completion_timeout(&data->fw_comp, msecs_to_jiffies(MXCNST_TIMEOUT30S))) {
                    ...
                    if ((pCmd->SQ.opCode == MEMX_ADMIN_CMD_FLASH_RW) && (pCmd->SQ.subOpCode == FLASH_ACCESS_ID_WRITE)) {
                        status = memx_admin_fetch_result(pCmd, data);
                        if ((pCmd->CQ.status != ERROR_STATUS_NO_ERROR) || (status != STATUS_COMPLETE)) {
                            pr_err("%s: return with error CQ.status %d.%d\n", __func__, pCmd->CQ.status, status);
                            goto done;
                        }
                        status = _admin_flash_access(pCmd, data);
                    }
                    status = memx_admin_fetch_result(pCmd, data);
                }
            }
    ...
    } else if ((pCmd->SQ.opCode == MEMX_ADMIN_CMD_FLASH_RW) && (pCmd->SQ.subOpCode == FLASH_ACCESS_ID_READ)) {
        if (copy_to_user(pCmd->SQ_flash.data_ptr, data->fw_rbuffer + sizeof(struct transport_cmd), pCmd->SQ.cdw4)) {
            pr_err("_admin_flash_access copy_to_user failed!");
            status = STATUS_COPY_ERROR;
        }
    }
...
static long _admin_get_feature(struct memx_data *memx_dev, struct transport_cmd *pCmd)
{
    ...
    switch (pCmd->SQ.subOpCode) {
        ...
        case FID_DEVICE_HW_INFO:
            pCmd->CQ.data[0] = memx_dev->hw_info.chip.generation;
            pCmd->CQ.data[1] = memx_dev->hw_info.chip.total_chip_cnt;
            pCmd->CQ.data[2] = memx_dev->hw_info.chip.curr_config_chip_count;
            pCmd->CQ.data[3] = memx_dev->hw_info.chip.group_count;
...
static uint32_t _admin_flash_data_transfer_to_device(struct memx_data *data, uint32_t length)
{
    uint32_t status = STATUS_COMPLETE;

    usb_fill_bulk_urb(data->fw_txurb, data->udev, usb_sndbulkpipe(data->udev, MEMX_FW_OUT_EP),
                        data->fw_wbuffer, length, memx_complete, data);

    if (usb_submit_urb(data->fw_txurb, GFP_KERNEL) < 0) {
        pr_err("%s:Can't submit admin flash transfer TX URB\n", __func__);
        status = STATUS_SUBMIT_ERROR;
    } else {
        if (!wait_for_completion_timeout(&data->fw_comp, msecs_to_jiffies(MXCNST_TIMEOUT30S))) {
            pr_err("%s:Admin flash transfer TX URB timeout\n", __func__);
            status = STATUS_WRITE_TIMEOUT;
        }
    }
...
{
    uint32_t status = STATUS_COMPLETE;
    uint32_t length = pCmd->SQ.cdw4;

	if (copy_from_user(data->fw_wbuffer, pCmd->SQ_flash.data_ptr, length)) {
		pr_err("_admin_flash_access copy_from_user failed!");
		status = STATUS_COPY_ERROR;
	} else
		status = _admin_flash_data_transfer_to_device(data, length);
    ...
    switch (pCmd->SQ.opCode) {
        ...
        case MEMX_ADMIN_CMD_DEVIOCTRL:
            if (pCmd->SQ.subOpCode == FID_DEVICE_I2C_TRANSCEIVE) {
                ...
                ret = -EFAULT;
                pr_err(" %s: non-support admin cmd(%u) sub(%u)\n", __func__, pCmd->SQ.opCode, pCmd->SQ.subOpCode);
            }
            break;
        case MEMX_ADMIN_CMD_FLASH_RW:
            ret = memx_admin_trigger(pCmd, memx_dev);
            break;
        ...
        default:
            ret = -EFAULT;
            pr_err(" %s: non-support admin cmd(%u)\n", __func__, pCmd->SQ.opCode);
            break;
    }
