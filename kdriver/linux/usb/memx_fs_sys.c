...
static char *g_usage[19] = {
	"Usage: echo \"fwlog chip_id[0-7] [hex_addr] [hex_val]\" > /sys/memxusb[dev_id 0-3]/cmd\n",
	"==========================================================================================\n",
	"Ex: Dump chip(0) firmware log\n",
	"echo \"fwlog 0\" > /sys/memx0/cmd; sudo dmesg\n",
	"Ex: Dump chip(1) firmware log\n",
	"echo \"fwlog 1\" > /sys/memx0/cmd; sudo dmesg\n\n",

	"Ex: Read chip(0) address 0x400A0000\n",
	"echo \"read 0 0x400A0000\" > /sys/memx0/debug; sleep 1; echo \"fwlog 0\" > /sys/memx0/cmd; sudo dmesg\n",
	"Ex: Write chip(0) address [0x400A0000]=0x12345678\n",
	"echo \"write 0 0x400A0000 0x12345678\" > /sys/memx0/debug; sleep 1; echo \"fwlog 0\" > /sys/memx0/cmd; sudo dmesg\n\n",
...
static char *debug_usage[5] = {
	"Usage: echo \"rmtcmd chip_id[0-7] [hex_cmdcode] [hex_cmdparam]\" > /sys/memxusb[dev_id 0-3]/debug\n",
	"==========================================================================================\n",
...
static ssize_t update_flash_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (request_firmware(&firmware, FIRMWARE_BIN_NAME, &memx_dev->udev->dev) < 0) {
		len = sprintf(to_user_buf_pos, "%s", "downlaod_fw: request_firmware for cascade.bin failed\n"); to_user_buf_pos += len; res += len;
		return 0;
	}
	...
	do {
		...
		if (time_after(jiffies, timeout) || (ret != 0)) {
			len = sprintf(to_user_buf_pos, "Update QSPI FLASH TIMEOUT FAILED!!\n"); to_user_buf_pos += len; res += len;
...
static ssize_t verinfo_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	len = sprintf(to_user_buf_pos, "usb intf device:\n");
	to_user_buf_pos += len;
	res += len;
	...
	//usb_control_msg	(memx_dev->udev, 0, 0x3, USB_DIR_OUT | USB_TYPE_STANDARD | USB_RECIP_DEVICE, 2,0x0400, 0, 0, 1000);
	return res;
}
	...
	if (!ret) {
		for (chip_id = 0; chip_id < memx_dev->chipcnt; chip_id++) {
			if (buffer[chip_id] != 0xFF) {
				len = sprintf(to_user_buf_pos, "chip%d(group%d):%u%% ", chip_id, grpid, buffer[chip_id]);
				to_user_buf_pos += len;
				res += len;
				grpid++;
			}
...
static ssize_t temperature_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (!ret) {
		for (chip_id = 0; chip_id < memx_dev->chipcnt; chip_id++) {
			...
			len = sprintf(to_user_buf_pos, "CHIP(%d) PVT%d Temperature: %d C  (%u Kelvin) (ThermalThrottlingState: %d)\n", chip_id, (data>>16)&0xF, temp_Celsius, (data&0xFFFF), (data>>20)&0xF);
			to_user_buf_pos += len;
			res += len;
		}
	}
...
static ssize_t thermalthrottling_store(struct kobject *kobj, struct kobj_attribute *attr, const char *user_input_buf, size_t user_input_buf_size)
{
	...
	if (!memx_dev) {
		pr_err("memx_sys_write: memx_dev is NULL!\n");
		return ret;
	}
	if (!user_input_buf) {
		pr_err("memx_proc_write: user input buf is NULL!\n");
		return ret;
	}
	if (user_input_buf_size == 0) {
		pr_err("Command length is invild!\n");
...
s32 memx_fs_sys_init(struct memx_data *memx_dev)
{
	...
	if (!memx_dev) {
		pr_err("memx_fs_sysfs_init: memx_dev is NULL\n");
		return -EINVAL;
	}
	...
	if (!memx_dev->fs.hif.sys.root_dir) {
		pr_err("memx_fs_sysfs init: create sys root_dir fail!!\n");
		return -ENOMEM;
	}
...
void memx_fs_sys_deinit(struct memx_data *memx_dev)
{
	if (!memx_dev) {
		pr_err("%s: memx_dev is NULL\n", __func__);
		return;
