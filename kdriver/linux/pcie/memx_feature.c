...
static long _admin_flash_access(struct memx_pcie_dev *memx_dev, struct transport_cmd *pCmd){
	long ret = 0;
	uint8_t chip_id = pCmd->SQ.cdw2;
	uint32_t length = pCmd->SQ.cdw4;

	if (pCmd->SQ.subOpCode == FLASH_ACCESS_ID_WRITE) {
		if (copy_from_user((void *)(memx_dev->mpu_data.rx_dma_coherent_buffer_virtual_base + OFMAP_EGRESS_DCORE_DMA_COHERENT_BUFFER_SIZE_512KB), pCmd->SQ_flash.data_ptr, length)) {
			pr_err("memryx: _admin_flash_access copy_from_user failed\n");
			ret = -EFAULT;
			goto done;
		}
		dma_sync_single_for_device(&memx_dev->pDev->dev, (dma_addr_t)(memx_dev->mpu_data.hw_info.fw.rx_dma_coherent_buffer_base + OFMAP_EGRESS_DCORE_DMA_COHERENT_BUFFER_SIZE_512KB), IFMAP_INGRESS_DCORE_DMA_COHERENT_BUFFER_SIZE_512KB, DMA_BIDIRECTIONAL);
	}
    ...
    if (chip_id < memx_dev->mpu_data.hw_info.chip.total_chip_cnt) {
		...
		pr_err("memryx: chip id error, chip_id.total_chip_cnt %d.%d\n", chip_id, memx_dev->mpu_data.hw_info.chip.total_chip_cnt);
		pCmd->CQ.status = ERROR_STATUS_PARAMETER_FAIL;
	}

	if ((pCmd->CQ.status == ERROR_STATUS_NO_ERROR) && (pCmd->SQ.subOpCode == FLASH_ACCESS_ID_READ)) {
		dma_sync_single_for_cpu(&memx_dev->pDev->dev, (dma_addr_t)(memx_dev->mpu_data.hw_info.fw.rx_dma_coherent_buffer_base + OFMAP_EGRESS_DCORE_DMA_COHERENT_BUFFER_SIZE_512KB), IFMAP_INGRESS_DCORE_DMA_COHERENT_BUFFER_SIZE_512KB, DMA_BIDIRECTIONAL);
		if (copy_to_user(pCmd->SQ_flash.data_ptr, (void *)(memx_dev->mpu_data.rx_dma_coherent_buffer_virtual_base + OFMAP_EGRESS_DCORE_DMA_COHERENT_BUFFER_SIZE_512KB), length)) {
			pr_err("memryx: _admin_flash_access copy_to_user failed\n");
			ret = -EFAULT;
			goto done;
		}
	}
...
static long _admin_command(struct memx_pcie_dev *memx_dev, struct transport_cmd *pCmd)
{
	...
	case MEMX_ADMIN_CMD_FLASH_RW:
		ret = _admin_flash_access(memx_dev, pCmd);
		break;
	default:
		ret = -EFAULT;
		pr_err("memryx: %s: unsupported admin cmd(%u)\n", __func__, pCmd->SQ.opCode);
		break;
	}
