...
static const struct file_operations memx_pcie_fops = {
    .owner          = THIS_MODULE,
...
static s32 memx_pcie_probe(struct pci_dev *pDev, const struct pci_device_id *id)
{
	...
	ret = pci_request_regions(pDev, "memx_ai_chip");
	if (ret) {
	...
	char_dev = device_create(g_char_device_class, &pDev->dev, MKDEV(MAJOR(g_memx_devno), memx_dev->minor_index), memx_dev, DEVICE_NODE_NAME, memx_dev->minor_index);
	if (IS_ERR(char_dev)) {
		pr_err("memryx: failed createing memx device node(%d)\n", memx_dev->minor_index);
		ret = PTR_ERR(char_dev);
		goto err_dev_init;
	}
	memx_dev->char_dev = char_dev;
	...
	feature_dev = device_create(g_char_device_class, &pDev->dev, MKDEV(MAJOR(g_feature_devno), memx_dev->minor_index), memx_dev, DEVICE_NODE_NAME "_feature", memx_dev->minor_index);
...
static s32 __init memx_pcie_module_init(void)
{
	...
	ret = alloc_chrdev_region(&g_memx_devno, 0, MAX_CHIP_NUM, DEVICE_CLASS_NAME);
	if (ret < 0) {
	...
	if (IS_ERR(g_char_device_class)) {
		ret = PTR_ERR(g_char_device_class);
		pr_err("memryx: module_init: failed to call class_create %d\n", ret);
		return ret;
	}
