 ...
 *  z: minor bug fix.
 */
#define MEMX_LIBRARY_VERSION "2.13.0"
...
typedef enum _memx_flash_access_opcode {
  OPCODE_FLASH_ACCESS_READ  = 0,
  OPCODE_FLASH_ACCESS_WRITE = 1,
  OPCODE_FLASH_ACCESS_ERASE = 2,
  OPCODE_FLASH_ACCESS_MAX
} memx_flash_access_opcode;
 ...
 *
 * @param model_id            model ID
 * @param chip_gen            chip gen of device (MEMX_MPU_CHIP_GEN_CASCADE_PLUS)
 *
 * @return 0 on success, otherwise error code
 */
...
/**
 * @brief Write vendor ID in device flash.
 *
 * @param group_id Group(device) ID.
 * @param length   Vendor ID length.
 * @param buffer   User buffer pointer for vendor ID.
 *
 * @return 0 on success, otherwise error code
 */
MEMX_API_EXPORT memx_status memx_write_vendor_id(uint8_t group_id, uint32_t length, void* buffer);

/**
 * @brief Read vendor ID from device flash.
 *
 * @param group_id Group(device) ID.
 * @param length   Vendor ID length.
 * @param buffer   User buffer pointer for vendor ID.
 ...
 * @return 0 on success, otherwise error code
 */
MEMX_API_EXPORT memx_status memx_read_vendor_id(uint8_t group_id, uint32_t length, void* buffer);

/**
 * @brief Erase vendor ID in device flash.
 *
 * @param group_id Group(device) ID.
 * @param length   Vendor ID length.
 *
 * @return 0 on success, otherwise error code
 */
MEMX_API_EXPORT memx_status memx_erase_vendor_id(uint8_t group_id, uint32_t length);
